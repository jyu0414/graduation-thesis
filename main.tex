...
% 日本語情報（必要なら）
\jclass  {卒業論文}                             % 論文種別
\jtitle    {時系列UX評価手法に関する研究}    % タイトル。改行する場合は\\を入れる
\juniv    {慶應義塾大学}                  % 大学名
\jfaculty  {環境情報学部}               % 学部、学科
\jauthor  {佐々木 雄司}                       % 著者
\jhyear  {3}                                   % 平成○年度
\jsyear  {2021}                                 % 西暦○年度
\jkeyword  {ユーザビリティ，UX, 脈波}     % 論文のキーワード
\jproject{全世界インタフェースデザイン (増井研究会)} %プロジェクト名
\jdate{2022年1月}

% 英語情報（必要なら）
\eclass  {Graduation Thesis}                            % 論文種別
\etitle    {Studies on The Sequential User Experience Evaluation Method}      % タイトル。改行する場合は\\を入れる
\euniv  {Keio University}                             % 大学名
\efaculty  {Faculty of Environment and Information Studies}  % 学部、学科
\eauthor  {Sasaki, Yuji}                           % 著者
\eyear  {2021}                                        % 西暦○年度
\ekeyword  { Usability, UX, Pulse Wave }          % 論文のキーワード
